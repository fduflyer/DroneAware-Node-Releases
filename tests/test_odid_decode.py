...
class LocationMessage(unittest.TestCase):
    ...
    def test_unknown_altitudes_are_null(self):
        # Bytes 15-16 (geodetic) and 17-18 (height) both 0 = F3411 "unknown".
        # They were published as -1000.0 m, which the UI showed as -3281 ft.
        msg = bytearray(LOC_HOVER)
        msg[15:19] = b"\x00\x00\x00\x00"
        for name, f in FEEDERS:
            with self.subTest(feeder=name):
                d = f.parse_location(bytes(msg))
                self.assertIsNone(d["altitude_geo"])
                self.assertIsNone(d["height_agl"])
...
class SnapshotTrail(unittest.TestCase):
    ...
    def test_zero_zero_is_not_a_trail_point(self):
        ...
        store.add({"t": now - 2, "mac": mac, "type": "Location/Vector", "lat": 0.0, "lon": 0.0,
                   "alt": -1000.0, "height_agl": -1000.0})
        store.add({"t": now - 1, "mac": mac, "type": "Location/Vector", "lat": 40.1234641, "lon": -75.4568655,
                   "alt": 226.0, "height_agl": 30.0})
        snap = store.snapshot()
        trail = snap["macs"][0]["trail"]
        self.assertEqual(trail, [[40.1234641, -75.4568655, 30.0]])

    def test_trail_altitude_is_height_not_sea_level(self):
        # The browser appends live points by height_agl; a reloaded trail
        # must use the same altitude or the two halves of one flight sit
        # hundreds of meters apart and the geodetic half is labelled AGL.
        try:
            import web_ui
        ...
        store = web_ui.DetectionStore()
        now = time.time()
        mac = "8c:1e:d9:00:00:02"
        store.add({"t": now - 3, "mac": mac, "type": "Location/Vector", "lat": 40.1, "lon": -75.1,
                   "alt": 262.0, "height_agl": 32.5})
        store.add({"t": now - 2, "mac": mac, "type": "Location/Vector", "lat": 40.2, "lon": -75.2,
                   "alt": 231.0, "height_agl": -1000.0})   # old spool: unknown height
        store.add({"t": now - 1, "mac": mac, "type": "Location/Vector", "lat": 40.3, "lon": -75.3,
                   "alt": 231.0})                          # no height broadcast at all
        trail = store.snapshot()["macs"][0]["trail"]
        self.assertEqual([p[2] for p in trail], [32.5, None, None])
