...
def _restore_system_library_path() -> None:
    ...
    (Bookworm), so on Trixie `systemctl` dies before it starts
    ("libcrypto.so.3: version `OPENSSL_3.4.0' not found") -- which is what the
    BLE status fallback's `systemctl is-active` has been hitting. (sudo is
    unaffected: it is setuid, so the loader ignores LD_LIBRARY_PATH.)
